...
const episodeFilter = $('#episode-filter');
...
function selectLine(line, updateUrl = true) {
  ...
  if (!source) {
    ...
    const seek = () => {
      video.currentTime = clipRange(line).start;
...
function render() {
  const matches = filterLines(lines, query.value, episodeFilter.value);
...
episodeFilter.addEventListener('change', resetResults);
...
try {
  ...
  for (const episode of availableEpisodes) {
    ...
    option.textContent = episodes[episode]?.title || episode;
